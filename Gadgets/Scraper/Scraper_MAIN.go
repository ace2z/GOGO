...
//					R.Println(" Error! Have Redirect: ", http.ErrUseLastResponse)
	...
	// Gets all the request headers that were sent
	var headers = ""
	if reqHeadersBytes, err := json.Marshal(MAIN_REQ.Header); err != nil {
		Y.Println(" WARNING: Cant show request Headers in SCRAPER_TOOL")
		W.Println(err)
	} else {
	...
	return true, doc, FULL_RESPONSE_TEXT, headers
